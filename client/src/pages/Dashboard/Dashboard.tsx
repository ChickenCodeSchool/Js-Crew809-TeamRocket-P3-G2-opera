...
interface ItemsPerDay { day: string; totalItems: number; timestamp: number; }
interface CustomersPerDay { day: string; totalCustomers: number; timestamp: number; }

interface RawItemsData { day: string; totalItems: number; }
interface RawCustomersData { day: string; totalCustomers: number; }
    ...
    async function fetchAll() {
      try {
        ...
        if (isMounted) {
          setRevenue(rev);
          setTopProducts(prod);
          setItemsPerDay(items.map(i => ({
            ...i,
            timestamp: new Date(i.day).getTime()
          })));
          setCustomersPerDay(customers.map(c => ({
            ...c,
        ...
        <section className="dashboard-section dashboard-right">
          <ResponsiveContainer width="100%" height={450}>
            <BarChart data={topProducts} margin={{ top: 30, right: 30, left: 20, bottom: 20 }}>
              ...
              <XAxis dataKey="name" tick={false} axisLine={{ stroke: "#333" }} />
              <YAxis tick={{ fill: SILVER_MUTED }} axisLine={false} tickLine={false} />
              <Tooltip
      ...
      <div className="dashboard-bottom">
        <section className="dashboard-section">
            ...
            <AreaChart data={customersPerDay}>
              <defs>
                <linearGradient id="areaCustomers" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={SILVER_MUTED} stopOpacity={0.2} />
                  <stop offset="100%" stopColor={SILVER_MUTED} stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid stroke="#2D2D2D" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="timestamp" type="number" scale="time" tick={false} axisLine={false} domain={["dataMin", "dataMax"]} />
              <YAxis tick={{ fill: SILVER_MUTED, fontWeight: 500 }} axisLine={false} tickLine={false} />
              <Tooltip
          ...
          <div className="section-footer">
            ...
            <p className="section-kpi">{totalCustomers.toLocaleString()} clients</p>
          </div>
        </section>
        ...
        <section className="dashboard-section">
          ...
          <div className="section-footer">
            ...
            <p className="section-kpi">{totalItems.toLocaleString()} articles</p>
          </div>
        </section>
      </div>
    </div>
